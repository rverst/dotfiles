...
**Runtime secrets** are deliberately *not* handled by Age. `local/dot-local/scripts/secret` (stowed to `~/.local/scripts`, already on `PATH`) stores API tokens in the OS keyring (`security` on macOS, `secret-tool`/libsecret on Linux, `pwsh.exe` + PowerShell SecretManagement over WSL interop on WSL2; no backend on servers → clean error) and injects them into a single child process via `secret run NAME... -- cmd`, so they never sit in cleartext on disk nor get exported globally. The WSL bridge keeps values off argv: the PowerShell snippet travels in `-EncodedCommand`, verb/name in env vars forwarded via `WSLENV`, and the value only on stdin/stdout base64-encoded both ways; `secret run` batches all names into one interop call because each round trip costs a few hundred ms. PowerShell 5.1 is refused outright. `secret sync` pulls from a vault into the keyring; two vault backends are supported behind one interface (`vault_backend`/`vault_prepare`/`vault_get`/`vault_list`): `rbw` for Bitwarden/Vaultwarden and `keepassxc-cli` for a local `.kdbx` (configured via `SECRET_KEEPASS_DB`/`_KEYFILE`/`_YUBIKEY`/`_NO_PASSWORD` in `~/.localrc`). Both follow the same layout convention — a folder/group (`SECRET_VAULT_FOLDER`, default `dotfiles`) holding one item per secret, item name == secret name, value in the password field (`SECRET_VAULT_FIELD` overrides) — which is what makes `secret sync --all` and `--list` possible; items with names that aren't valid secret names are skipped with a warning. `vault_prepare` runs the unlock in the **parent** shell (rbw agent, or the KeePass passphrase read once and piped to each `keepassxc-cli` call on stdin) because `vault_get` is invoked inside command substitutions, and a subshell would re-prompt per item. Config files reference the variable (`${NPM_REGISTRY_TOKEN}`, `${env.GITLAB_TOKEN}`) instead of embedding the value. Tool wrappers live at the bottom of `~/.localrc` (below its `PATH` exports, since `$+commands` guards resolve at definition time). See the README's "Runtime secrets" section, including the known gap around `./mvnw` and IDE-launched builds.

**Runtime versions.** `local/dot-local/scripts/{jdk-home,node-bin}` discover installed JDKs/Node versions at call time (no hardcoded lists) and print a path for a requested version. They are plain `sh` on purpose: both the zsh autoload functions `config/dot-config/zsh/functions/{setJava,setNode}` *and* the bash `config/dot-config/direnv/direnvrc` (`use java` / `use node`) call them. Switching strips the previously selected `PATH` entry before prepending, so it's idempotent. Machine defaults are pinned in the flavour's `dot-localrc` (`setJava -q 21`). `node-bin` deliberately resolves the Cellar version behind each keg because Homebrew aliases all unreleased `node@N` at the current unversioned keg. Both scripts are in the CI shellcheck list.
